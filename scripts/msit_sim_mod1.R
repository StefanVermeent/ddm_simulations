...
# Read DDM results
sim_msit_ddm <- read_DDM(task = "msit", path = "data/msit", model_version = "_mod1") |>
  summarise(
...
msit_ddm_gt <-
  ...
  # Remove very small non-decision times because they cause issues with fitting the model (and are biologically not very plausible)
  filter(a_gt > 0.2 & t0_con_gt > 0.01 & t0_inc_gt > 0.01) |>
  mutate(id = 1:n()) |>
...
model_msit <- "model {
  ...
  for (s in 1:nSubjects) {
    ...
    alpha[s]  ~ dnorm(muAlpha, precAlpha) T(.05, 10)
  }
...
ddm_msit_traces_mod1 <- mcmc_msit |>
...
save(ddm_msit_traces_mod1, ddm_msit_data_mod1, ddm_msit_cor_mod1, file = "analysis_objects/ddm_msit_mod1_results.RData")

remove_DDM_files(path = "data/msit")
