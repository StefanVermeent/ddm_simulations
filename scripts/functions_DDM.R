...
fast_dm_settings <- function(task, path = "", model_version = "", method = "ml", precision = 3, zr = 0.5, d = 0, szr = 0, sv = 0, st0 = 0, p = 0, depend = "", format) {
  ...
  write_file(str_c(spec, collapse = "\n"), file.path(path, glue("{task}{model_version}_ml.ctl")))
}

write_DDM_files <- function(path = "", data, vars = c("rt", "correct"), task) {
  ...
  for (i in unique(data$id)) {
    ...
    change_DDM_i <- data %>%
      filter(id == i) %>%
      select(all_of(vars)) %>%
      write_delim(file = file.path(path, str_c(task, "_DDM_subject", i, ".dat")), col_names = FALSE)
  }
}
...
execute_fast_dm <- function(task, path = "", model_version = "") {
  # 2. Run DDM model
  path_to_files <- paste0("pushd ", file.path(getwd(), path)) %>% str_replace_all(., pattern="/", replacement="\\\\")
...
read_DDM <- function(task, path, model_version = "") {
...
remove_DDM_files <- function(path) {
  file.remove(list.files(path, pattern = ".dat$|.lst$", full.names=TRUE))
